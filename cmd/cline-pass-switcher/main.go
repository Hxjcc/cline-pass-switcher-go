...
// the only thing between the internet and full administrative access. Without a
// proxy key, a non-loopback peer is trusted because the operator declared that
...
func warnUnauthenticatedAccess(cfg model.Config) {
	...
	if cfg.TrustLocalPortForward {
		log.Print("[警告] 未设置代理密钥（proxyKey / PROXY_KEY），但已开启 TRUST_LOCAL_PORT_FORWARD：任何能连上该端口的来源，只要把 Host 头写成 127.0.0.1 或 localhost，就会被当作本机访问，并拥有完整管理权限（含读取账号密钥）。请确认端口只映射到宿主机回环地址，或设置代理密钥。（compose 默认把端口绑定在 127.0.0.1:3123，属于安全组合，这种情况可忽略本警告；一旦改动 ports 绑定或改用域名访问，就必须设置代理密钥。）")
	}
	if len(cfg.TrustedProxies) > 0 {
		log.Printf("[警告] 未设置代理密钥，但已信任反向代理 %s：来自这些地址的 X-Forwarded-For / X-Real-IP 会被采信。请确认这些代理确实无法被绕过，且不会把任意客户端声明成本机。", strings.Join(cfg.TrustedProxies, ","))
	}
}
